...
class CategoryController {
    ...
    static async restoreCategory(req, res) {
        try {
            ...
            return res.status(200).json({ message: 'Danh mục đã được phục hồi thành công', data: category });
        } catch (error) {
            return res.status(500).json({ message: 'Có lỗi xảy ra khi phục hồi danh mục' });
        }
    }
